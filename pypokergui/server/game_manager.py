...
class GameManager(object):
    ...
    def __init__(self):
        ...
        self.roundi = 0
        self.isSendMessgae = True

    def define_rule(self, max_round, initial_stack, small_blind, big_blind, blind_structure):
        self.rule = Engine.gen_game_config(max_round, initial_stack, small_blind, big_blind, blind_structure)
    ...
    def start_game(self):
        ...
        self.ai_players = {'0': FishPlayer(self.player_search)}#build_ai_players(self.members_info)
        self.engine = Engine.EngineWrapper()
        self.latest_messages = self.engine.start_game(players_info, self.rule,self,None)
    ...
    def update_game(self, action, amount):
        ...
        self.latest_messages = self.engine.update_game(action, amount,self)
        #print('table information:',self.latest_messages)
        self.next_player_uuid = fetch_next_player_uuid(self.latest_messages)
    ...
    def ask_action_to_ai_player(self, uuid):
        ...
        # print('player:',int(ask_uuid)+1)
        # print('information:',ask_message)
        # action, amount = ask_action(ask_uuid,ask_message)#Control.ask_action(ask_uuid, ask_message)''
